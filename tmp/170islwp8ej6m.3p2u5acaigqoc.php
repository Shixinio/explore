...
<div class="tableDiv">
    <table class="mdl-data-table mdl-js-data-table slimTable">
        ...
        <tr>
            ...
            <td class="mdl-data-table__cell--non-numeric">
                <strong>
                    ...
                    <div class="mdl-tooltip mdl-tooltip--right mdl-tooltip--large alignLeft" for="<?= $account['account'] ?>"> BTC: <?= number_format(($market["btc"] * str_replace("'", "", $account['effectiveBalanceNXT'])), 8, ".", "'") ?><br>USD: <?= number_format(($market["usd"] * str_replace("'", "", $account['effectiveBalanceNXT'])), 8, ".", "'") ?><br>EUR: <?= number_format(($market["eur"] * str_replace("'", "", $account['effectiveBalanceNXT'])), 8, ".", "'") ?> </div>
                </strong>
            </td>
        </tr>
        <tr>
            <td class="mdl-data-table__cell--non-numeric">Capacity</td>
            <td class="mdl-data-table__cell--non-numeric">
                <strong>
                    <span id="<?= $account['account'] ?>" class="underText"><?= $account['capacity'] ?> T</span>
                </strong>
            </td>
        </tr>
        <tr>
            <td class="mdl-data-table__cell--non-numeric">BestMortgage</td>
            <td class="mdl-data-table__cell--non-numeric">
                <strong>
                    <span id="<?= $account['account'] ?>" class="underText"><?= $account['bestmMrtgage'] ?> </span>
                </strong>
            </td>
        </tr>
        <tr>
            <td class="mdl-data-table__cell--non-numeric">MortgageState</td>
            <td class="mdl-data-table__cell--non-numeric">
                <strong>
                    <span id="<?= $account['account'] ?>" class="underText"><?= $account['mortgageState'] ?> </span>
                </strong>
            </td>
        </tr>
        <tr class="<?= count($account['assets'])?'':'hide' ?>">
            ...
            <td class="mdl-data-table__cell--non-numeric">
                <?php foreach (($account['assets']?:[]) as $asset): ?>
                    <a menuLink="assetsLink" href="/transaction/<?= $asset['asset'] ?>">#<?= $asset['asset'] ?> (<?= $asset['name'] ?>)</a><br>
                <?php endforeach; ?>
            </td>
        </tr>
        </tbody>
        ...
        <?php foreach (($account['transactions']?:[]) as $transaction): ?>
            <tr txtype="txType<?= $transaction['typed'] ?>" class="txRow">
                ...
                <td class="mdl-data-table__cell--non-numeric <?= $transaction['move'] ?> boldText">
                    ...
                    <?php if ($transaction['move'] == 'greenText'): ?>
                    ...
                    <?php endif; ?>
                    <?php if ($transaction['move'] == 'redText'): ?>
                                                <span class="mdl-chip mdl-chip--contact">
